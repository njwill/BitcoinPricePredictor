...
class AIAnalyzer:
    ...
    Key changes vs. your original:
      - Asset-agnostic via `asset_name`
      - Strict JSON output schema (no prose; no regex scraping)
      - Explicitly bans external context (news, sessions, open/close, etc.)
      - Low-temperature, deterministic settings
      - Robust handling of timezones/targets
      - Optional precomputed features to keep the model grounded in numbers
      - No trading recommendations; only evidence grounded in provided arrays

    Expects:
      - data_3m, data_1w: pandas.DataFrame with columns ['Open','High','Low','Close','Volume'].
        Index must be datetime-like (coerced if needed). Sorted ascending (coerced if needed).
        These represent ~3m and ~1w windows; if callers pass more, we trim internally.
        Optional: df.attrs['display_from_index'] (int) for trimming what you show in RECENT arrays.

      - indicators_3m, indicators_1w: dict-like of pandas.Series for technical indicators
        e.g., 'RSI','MACD','MACD_Signal','MACD_Histogram',
              'BB_Upper','BB_Lower','BB_Middle','EMA_20','SMA_50','SMA_200'

    Config via environment:
      - OPENAI_API_KEY: required
      - GPT5_MODEL: optional (default: "gpt-5")
      - AI_ANALYZER_DEBUG: "1" to enable verbose Streamlit debug messages
    """
    ...
    def __init__(self, debug: Optional[bool] = None):
        ...
        # cache last current_price for internal calculations if needed
    ...
    def generate_comprehensive_analysis(
        ...
        """
        Generates a strict, data-only JSON analysis + forecast via GPT.

        Returns a dict:
        {
          "status": "ok" | "insufficient_data" | "error",
          "model_json": <JSON returned by model or error object>,
          "probabilities": {... derived convenience fields ...},
          "timestamp": <iso8601>
        }
        """
        ...
        try:
            ...
            # If preparation determined we truly lack essentials
            if analysis_data.get("prep_status") == "insufficient_data":
                return {
                    "status": "insufficient_data",
                    "model_json": {
                        "status": "insufficient_data",
                        "notes": analysis_data.get("prep_notes", ["insufficient data during preparation"])
                    },
                    "probabilities": self._default_probs(),
                    "timestamp": datetime.now().isoformat(),
                }

            # Call the model (strict JSON expected)
    ...
    def _ensure_datetime_index(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # If numeric index, try to promote a date/time column to index
        if df.index.dtype.kind in ['i', 'f']:
            ...
            if date_cols:
                ...
                # Fallback: create a dummy daily index
    ...
    def _coerce_ohlcv_numeric(self, df: pd.DataFrame) -> pd.DataFrame:
        """Ensure OHLCV columns are numeric (handles strings with commas)."""
        if df is None or df.empty:
            return df
        df = df.copy()
    ...
    def _limit_to_days(self, df: pd.DataFrame, days: int) -> pd.DataFrame:
        """Return the slice covering the last `days` days."""
        if df is None or df.empty:
            return df
    ...
    def _annualization_sqrt(self, index: pd.Index) -> float:
        """Compute sqrt(periods-per-year) from index spacing (24/7 assets supported)."""
        try:
            if len(index) < 2:
                return 1.0
            dt = (index[1] - index[0])
    ...
    def _infer_index_step(self, index: pd.Index) -> Optional[pd.Timedelta]:
        """Infer a representative step from a datetime index."""
    ...
    def _determine_timezone(self, *indexes: pd.Index) -> pytz.BaseTzInfo:
        """Prefer tz from the first tz-aware index; else default to US/Eastern."""
        for idx in indexes:
    ...
    def _prepare_analysis_data(
        ...
        """Prepare and summarize all data for the model."""
        try:
            # Sanitize frames
            ...
            if target_datetime:
                ...
                # Optional fallback: predict one step ahead of the last known 1w bar (or 3m if 1w absent)
                base_df = window_1w if window_1w is not None and not window_1w.empty else window_3m
            ...
            # Basic sanity check vs latest 1w close
            if window_1w is not None and not window_1w.empty:
            ...
            def _summary(df: pd.DataFrame, label: str) -> Dict[str, Any]:
                ...
                return {
                    ...
                    "price_change_pct": float((actual_current_price - start_price) / (start_price if start_price else 1) * 100.0),
            ...
            data_3m_summary = _summary(window_3m, "3 months") if window_3m is not None and not window_3m.empty else {}
            data_1w_summary = _summary(window_1w, "1 week") if window_1w is not None and not window_1w.empty else {}

            # Indicators summary (uses actual current_price)
            indicators_summary = self._summarize_indicators(indicators_3m, indicators_1w, actual_current_price)

            # Enhanced chart arrays (compact tails for prompt)
            enhanced_data = self._prepare_enhanced_chart_data(
                window_3m, window_1w, indicators_3m, indicators_1w
            )

            # Override highs/lows from enhanced data to ensure consistency
            ...
            # Optional: precomputed numeric features to keep model grounded
    ...
    def _compute_features(
        ...
        """Optional numeric features for more grounded evidence."""
        feats: Dict[str, Any] = {}
    ...
    def _prepare_enhanced_chart_data(
        ...
        """Prepare arrays (tails) for the prompt; strictly from provided data."""
        try:
            ...
            if full_3m is not None and not full_3m.empty:
            ...
            if full_1w is not None and not full_1w.empty:
            ...
            recent_3m = full_3m.iloc[display_from_3m:] if (full_3m is not None and not full_3m.empty) else full_3m
            recent_1w = full_1w.iloc[display_from_1w:] if (full_1w is not None and not full_1w.empty) else full_1w

            tail_3m = recent_3m.tail(50) if recent_3m is not None and not recent_3m.empty else recent_3m
            tail_1w = recent_1w.tail(30) if recent_1w is not None and not recent_1w.empty else recent_1w
            ...
            enhanced["3m_data"] = {
                ...
                ) if full_3m is not None and not full_3m.empty else "N/A",
                "data_range": (
                    f"{pd.Timestamp(tail_3m.index[0]).strftime('%B %d')} to "
                    f"{pd.Timestamp(tail_3m.index[-1]).strftime('%B %d, %Y')}"
                ) if tail_3m is not None and not tail_3m.empty else "N/A",
                "period_highs_lows": {
                    "period_high": full_3m_high,
                    "period_low": full_3m_low,
                    "recent_high": float(recent_3m["High"].max()) if recent_3m is not None and not recent_3m.empty else None,
                    "recent_low": float(recent_3m["Low"].min()) if recent_3m is not None and not recent_3m.empty else None,
                },
                "recent_prices": (
                    {
                    ...
                    } if tail_3m is not None and not tail_3m.empty else {}
                ),
                "indicators": {},
            }

            # Fill 3m indicators (last 50)
            for indicator in [
            ...
            enhanced["1w_data"] = {
                ...
                ) if full_1w is not None and not full_1w.empty else "N/A",
                "data_range": (
                    f"{pd.Timestamp(tail_1w.index[0]).strftime('%B %d')} to "
                    f"{pd.Timestamp(tail_1w.index[-1]).strftime('%B %d, %Y')}"
                ) if tail_1w is not None and not tail_1w.empty else "N/A",
                "period_highs_lows": {
                    "period_high": full_1w_high,
                    "period_low": full_1w_low,
                    "recent_high": float(recent_1w["High"].max()) if recent_1w is not None and not recent_1w.empty else None,
                    "recent_low": float(recent_1w["Low"].min()) if recent_1w is not None and not recent_1w.empty else None,
                },
                "recent_prices": (
                    {
                    ...
                    } if tail_1w is not None and not tail_1w.empty else {}
                ),
                "indicators": {},
            }

            # Fill 1w indicators (last 30)
            for indicator in [
                "RSI","MACD","MACD_Signal","MACD_Histogram",
                "BB_Upper","BB_Lower","BB_Middle","EMA_20","SMA_50","SMA_200",
            ]:
            ...
            # Volume short-trend
            if full_3m is not None and not full_3m.empty:
                v50 = full_3m["Volume"].tail(50).mean()
                v10 = full_3m["Volume"].tail(10).mean()
            else:
                v50 = v10 = None

            if full_1w is not None and not full_1w.empty:
                v30 = full_1w["Volume"].tail(30).mean()
                v5 = full_1w["Volume"].tail(5).mean()
            else:
                v30 = v5 = None
    ...
    def _summarize_indicators(
        ...
        """Summarize indicators for the model; uses only provided arrays."""
    ...
    def _build_messages(self, analysis_data: Dict[str, Any], asset_name: str) -> Tuple[Dict[str, str], Dict[str, str]]:
        """Construct strict system+user messages for the Responses API."""
        # Forbidden topics are made explicit and non-negotiable
        system_content = (
            "You are a deterministic technical analyzer. Use ONLY the structured arrays provided. "
            "You are forbidden from using or inferring any external information, including: news, macro, earnings, "
            "on-chain data, funding/open interest, order book/liquidity, ETF flows, day-of-week effects, "
            "time-of-day/session effects, market open/close, weekends/holidays, or typical behavior. "
            "If needed data is missing/empty, return a JSON with status='insufficient_data' and list the missing fields."
        )

        # Compose the strict user message with embedded JSON payloads
        data_3m = analysis_data.get("data_3m", {})
        data_1w = analysis_data.get("data_1w", {})

        # The OUTPUT schema is the contract — exact, valid JSON, no prose.
        output_schema = {
            ...
            "p_up": "float 0..1 (prob price at target > CURRENT_PRICE)",
            "p_down": "float 0..1 (must satisfy p_up + p_down = 1)",
            "conf_overall": "float 0..1",
            "conf_price": "float 0..1",
            "expected_pct_move": "signed float percent; based on CURRENT_PRICE and predicted_price",
            "critical_levels": {"bullish_above": "number or null", "bearish_below": "number or null"},
            "evidence": [
                {
                    "type": "rsi|macd|bb|ema|price|volume|structure",
                    "timeframe": "3m|1w",
                    "ts": "YYYY-mm-dd HH:MM from provided arrays",
                    "value": "number or object of numbers (from arrays)",
                    "note": "short factual note; no sessions/news/macro"
                }
            ],
            "notes": ["optional short strings; include missing data if status=insufficient_data"]
        }
...
INDICATOR SNAPSHOT (already computed; do not change values):
{json.dumps(analysis_data.get('indicators', {}), indent=2)}

FEATURES (precomputed numerics to reference as evidence; optional):
{json.dumps(analysis_data.get('features', {}), indent=2)}

ENHANCED ARRAYS (use values ONLY from these arrays; you may cite index positions and timestamps):
...
- All numeric levels you cite must appear in the arrays or summaries above (or 'features').
- If any required array is empty or missing, return status='insufficient_data' with notes.

OUTPUT FORMAT:
Return EXACT, VALID JSON (no surrounding text). Schema example:
...
""".strip()
    ...
    def _generate_technical_analysis_gpt5(self, analysis_data: Dict[str, Any]) -> str:
        """Call GPT-5 with strict messages; expect EXACT JSON in response."""
        try:
            if not self.gpt5_client:
            ...
            return resp.output_text  # expected to be JSON
        except Exception as e:
    ...
    def _parse_json_response(self, response_text: str, current_price: float) -> Dict[str, Any]:
        """Parse and validate model JSON; clamp probabilities and compute move%."""
        try:
            ...
            data["status"] = status if status in ("ok", "insufficient_data") else "insufficient_data"

            # Clamp probs and ensure they sum to 1
            ...
            # Compute expected % move if we have a predicted_price
            ...
            if pred is not None and cp and cp > 0:
                try:
                    ...
                    data["expected_pct_move"] = data.get("expected_pct_move", None)
    ...
    def _extract_probabilities_from_json(self, data: Dict[str, Any], current_price: float) -> Dict[str, Any]:
        """Convenience struct similar to your previous output signature."""
