        ...
        class _OpenAIWrapper:
            """
            A robust OpenAI client wrapper that:
              - Tries Responses API and Chat Completions
              - Tries with and without seed
              - Tries fallback models if the primary fails
              - Captures last_error, last_model_used, and models_tried for diagnostics
            """
            def __init__(
                self,
            ...
            def _try_responses_api(self, model: str, system_msg: str, user_msg: str, allow_seed: bool) -> Optional[str]:
                kwargs = {
                    "model": model,
                    "input": [
                ...
                if allow_seed and self.seed is not None:
                    kwargs["seed"] = int(self.seed)
                resp = self.client.responses.create(**kwargs)
                return self._extract_text_from_responses(resp)

            def _try_chat_api(self, model: str, system_msg: str, user_msg: str, allow_seed: bool) -> Optional[str]:
                kwargs = {
                    "model": model,
                    "messages": [
                        {"role": "system", "content": system_msg},
                ...
                if allow_seed and self.seed is not None:
                    # Not all SDKs support a seed here; some will ignore or error.
                    kwargs["seed"] = int(self.seed)
            ...
            def generate(self, system_msg: str, user_msg: str) -> str:
                if not self.client:
                    self.last_error = "openai_client_not_initialized"
                    return ""
                ...
                for m in models_to_try:
                    # Try combinations: Responses/Chat x with/without seed
                    for api_kind in ("responses", "chat"):
                        for allow_seed in (True, False):
                            try:
                                self.models_tried.append(m)
                                if api_kind == "responses":
                                    text = self._try_responses_api(m, system_msg, user_msg, allow_seed)
                                else:
                                    text = self._try_chat_api(m, system_msg, user_msg, allow_seed)
                                if text and text.strip():
                                    self.last_model_used = m
                                    return text
                            except Exception as e:
                                self.last_error = f"{api_kind}_api_error(model={m}, allow_seed={allow_seed}): {e}"
                                if self.debug:
                                    st.warning(self.last_error)
                                # continue trying other combos/models
                                continue

                # All attempts failed
                if self.debug:
                    st.error(f"All OpenAI calls failed. Last error: {self.last_error}. Tried: {self.models_tried}")
                return ""
        ...
        class AIAnalyzer:
            ...
            def __init__(self, debug: Optional[bool] = None):
                ...
                # Fallback models via env (comma-separated)
                fb_env = os.getenv("GPT_FALLBACK_MODELS", "")
                fallback_models = [m.strip() for m in fb_env.split(",") if m.strip()] if fb_env else []
                self.gpt = _OpenAIWrapper(
                    self.openai_key,
                    self.model_name,
                    debug=self.debug,
                    temperature=0.0,
            ...
            def _prepare_analysis_data(
                ...
                try:
                    ...
                    # Determine tz from data else default to UTC. Use last data bar as 'current_time'
                    ...
                    enhanced_data = self._prepare_enhanced_chart_data(
                        window_3m, window_1w, indicators_3m, indicators_1w, step_3m, step_1w
                    )
                    ...
                    features = self._compute_features(
                        window_3m, window_1w, indicators_3m, indicators_1w, actual_current_price,
                        step_3m, step_1w, target, current_time
                    )
            ...
            def _prepare_enhanced_chart_data(
                ...
                try:
                    ...
                    # Tail sizes (env overrides). For smaller "nano/mini" models, keep tails modest to avoid token overflows.
                    tail_3m_env = os.getenv("ANALYZER_TAIL_3M", "")
                    tail_1w_env = os.getenv("ANALYZER_TAIL_1W", "")
                    default_tail_3m = 160 if ("nano" in (self.model_name or "").lower() or "mini" in (self.model_name or "").lower()) else 200
                    default_tail_1w = 80 if ("nano" in (self.model_name or "").lower() or "mini" in (self.model_name or "").lower()) else 100
                    try:
                        tail_points_3m = int(tail_3m_env) if tail_3m_env.strip().isdigit() else default_tail_3m
                    except Exception:
                        tail_points_3m = default_tail_3m
                    try:
                        tail_points_1w = int(tail_1w_env) if tail_1w_env.strip().isdigit() else default_tail_1w
                    except Exception:
                        tail_points_1w = default_tail_1w
                    ...
                    # Volume analysis (legacy fields kept for UI continuity)
            ...
            def _generate_technical_analysis_gpt5(self, analysis_data: Dict[str, Any]) -> str:
                ...
                if not raw.strip():
                    # Surface richer diagnostic info when model returns empty
                    diag = {
                        "status": "insufficient_data",
                        "notes": [
                            "empty_model_response",
                            f"openai_last_error={self.gpt.last_error}",
                            f"models_tried={self.gpt.models_tried}",
                            f"active_model={self.gpt.last_model_used or self.model_name}",
                            "tip:set AI_ANALYZER_DEBUG=1, verify OPENAI_API_KEY, set GPT5_MODEL to a valid model, or provide GPT_FALLBACK_MODELS",
                        ],
                    }
